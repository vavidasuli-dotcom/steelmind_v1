...
fun InventoryScreen(nav: NavController, db: AppDatabase) {
    ...
    fun refresh() {
        scope.launch(Dispatchers.IO) {
            stock = db.dao().listInventory()
            offcuts = db.dao().listOffcuts()
        }
    }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Raktárkészlet & Maradékok") }) }) { pad ->
        Column(Modifier.padding(pad).padding(16.dp)) {
            ...
            Button(onClick = {
                scope.launch(Dispatchers.IO) {
                    db.dao().upsertInventoryBar(InventoryBarEntity(lengthMm = length.toIntOrNull() ?: 0, count = count.toIntOrNull() ?: 0))
                    refresh()
                }
