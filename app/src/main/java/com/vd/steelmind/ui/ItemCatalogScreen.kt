...
fun ItemCatalogScreen(nav: NavController, db: AppDatabase) {
    ...
    fun refresh() { scope.launch(Dispatchers.IO) { items = db.dao().listItemTypes() } }
        ...
        Column(Modifier.padding(pad).padding(16.dp)) {
            ...
            Button(onClick = {
                scope.launch(Dispatchers.IO) {
                    db.dao().insertItemType(ItemTypeEntity(name = name, materialType = material.ifBlank { null }, priceFtPerM = price.toIntOrNull() ?: 0))
