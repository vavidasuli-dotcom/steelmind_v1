...
fun PlanScreen(nav: NavController, db: AppDatabase, projectId: Long) {
    ...
    LaunchedEffect(Unit) {
        scope.launch(Dispatchers.IO) {
            project = db.dao().listProjects().find { it.id == projectId }
            stock = db.dao().stockOptions(projectId)
            demands = db.dao().listDemands(projectId)
            inventory = db.dao().listInventory()
            items = db.dao().listItemTypes()
        }
        // read pdfBars from cut settings
    ...
    fun generate() {
        ...
        if (p.useInventory) {
            inventory.forEach { s -> stockMap[s.lengthMm] = s.count }
        }
    ...
    fun saveOffcuts() {
        ...
        scope.launch(Dispatchers.IO) {
            pr.bars.map { it.offcut }.filter { it >= p.minOffcutMm }.groupingBy { it }.eachCount().forEach { (len, cnt) ->
                db.dao().upsertOffcut(OffcutEntity(lengthMm = len, count = cnt))
            }
        }
    }

    fun totalCostFt(): Int {
        // cost from net cut lengths per item type
        val itemsById = items.associateBy { it.id }
        return demands.sumOf { d ->
            val pricePerM = itemsById[d.itemTypeId]?.priceFtPerM ?: 0
            (d.lengthMm * d.quantity) * pricePerM / 1000 // mm->m
        }
    }
